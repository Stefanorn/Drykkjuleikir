            ...
            foreach (Touch touch in Input.touches)
            {
                ...
                if (touch.phase == TouchPhase.Ended)
                {
                    ...
                    if (Physics.Raycast(ray, out hit, Mathf.Infinity))
                    {
                        ...
                        //Run the MouseUp() function on the clicked object
                        hit.transform.gameObject.SendMessage("MouseUp", hit.point, SendMessageOptions.DontRequireReceiver);
                    }
                }
            }
        }
        else //We are on a computer (more than likely)
        {
            if (Input.GetMouseButtonDown(0))  //Check to see if we've clicked
            {
                ...
                if (Physics.Raycast(ray, out hit, Mathf.Infinity))
                {
                    ...
                    //Run the MouseDown() function on the clicked object
                    hit.transform.gameObject.SendMessage("MouseDown", hit.point, SendMessageOptions.DontRequireReceiver);
                }
            }
